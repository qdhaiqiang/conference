...
<script type="text/javascript">
		...
		function loadInfoByQrcode(){
			$.ajax({
				...
				url : "<%=basePath%>checkinuser/fforCheckout/"+$("#userid").val(),
				type : "get",
				success : function(data) {
					//alert(data.key1);
					if(data.OperatorState == "false"){
						if(data.Message == "该用户没有注册该会议日程"){
							$("#state").val("0");//签出失败，状态为0
							$("#isHeadset").val("0");
							$("#isHeadset").prop("checked", false);
							$("#useridCopy").prop("value", $("#userid").val());
							clear();
							alert(data.Message);
							checkOut();//没有注册，同样记录一笔签入失败的记录
							...
							alert(data.Message);
							checkOut();//没有注册，同样记录一笔签入失败的记录
						}else if(data.Message == "该用户没有正常签入"){
							$("#state").val("1");
						...
						if(data.ifHeadset == "true"){
							$("#isHeadset").prop("checked", true);
				 			$("#isHeadset").prop("value", -1);
				 			$("#isHeadset").prop("disabled", false);
						}else if(data.ifHeadset == "false"){
							$("#isHeadset").prop("checked", false);
				 			$("#isHeadset").val("0");
				 			$("#isHeadset").prop("disabled", true);
				 			alert("该用户已签出且已归还耳机");
		...
		function loadInfoByMail(){
			//alert($("#mail").val());
			$.ajax({
				url : "<%=basePath%>checkinuser/fforCheckoutbymail?mail="+$("#mail").val(),
    ...
    <div class="query">
			<table>
				<tbody>
					...
					<tr>
						<td  width="200">
							<input name="userId" type="text" id="userid"  class="sign_control" />
						</td>
						<td>
					...
					<tr>
						<td colspan="2" style="text-align:left;"><label>邮箱签出:</label></td>
					</tr>
					<tr>
					...
					<tr>
						<td >	
							<input type="checkbox" name="isHeadset" id="isHeadset"><label>归还同声传译耳机</label>
							<input name="checkState" id="checkState" type ="hidden" value ="-1"/>
							<input name="state" id="state" type ="hidden" value ="1"/>
						</td>
						<td style="text-align:center;padding-top:10px;">	
		
							<label id="checkin" class="ljm_search">签出</label>	
						</td>
					</tr>
				</tbody>
			</table>
